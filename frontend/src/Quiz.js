...
useEffect(() => {
  ...
  const fetchQuiz = () => {
    ...
    if (!storyText) {
      setError('No story found to generate quiz.');
      setLoading(false);
      return;
    }
  ...
  const resetQuiz = () => {
    ...
    // Option 1: Use same questions again
    // setQuestions([...questions]);

    // Option 2: Fetch new quiz questions
    fetchQuiz();
  };
      ...
      <div className="quiz-container">
        ...
        {!loading && showResults && (
          <div className="results">
            <h3>{getText('Your Results:', 'आपका परिणाम:')}</h3>
            <p>
              {getText('Score:', 'अंक:')}{' '}
              {
            ...
            <button className="try-again-btn" onClick={resetQuiz}>
              {getText('Try Again', 'फिर से प्रयास करें')}
            </button>
          </div>
        )}
      </div>
